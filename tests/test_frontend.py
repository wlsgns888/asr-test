...
def test_root_serves_frontend_shell() -> None:
    ...
    assert "너는 전문 회의록 작성자다" in response.text
    assert "회사 내부 공유용 회의록" in response.text
    assert "회의 목적" in response.text
    assert "| 할 일 | 담당자 | 기한 | 비고 |" in response.text
    assert "담당자나 기한이 명확하지 않으면 “미정”으로 표시" in response.text
    assert "[여기에 녹취록 붙여넣기]" not in response.text
